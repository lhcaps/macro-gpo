...
- **D-14-10:** Runtime paths via `sys.frozen` detection (already implemented in `zedsu_backend.py` lines 26-34): `config.json` next to exe, `logs/` subdirectory
...
- **D-14-13:** Add app icon to bundle: `src/ZedsuFrontend/icons/app_icon.ico` → `icon` field in `tauri.conf.json` (currently empty)
...
- **Tray icons** at `src/ZedsuFrontend/icons/`: 10 PNG files already generated by `generate_icons.py`. Need to create `app_icon.ico` for bundle.
- **`sys.frozen` path resolution** in `zedsu_backend.py`: Already handles both dev and PyInstaller frozen modes.
...
- Icon: generate `app_icon.ico` from existing `generate_icons.py` / `generate_app_icon.py` scripts, reference in `tauri.conf.json`
