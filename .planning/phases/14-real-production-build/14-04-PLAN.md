...
- Exit code 4 when backend does not respond on port 9761
...
- `scripts/build_all.ps1` exits with non-zero code if any step fails
- `scripts/smoke_test_dist.py` verifies `dist/Zedsu/` layout
- `scripts/smoke_test_dist.py` starts `ZedsuBackend.exe`, waits for port 9761, checks `/health`
