# 12-5-06-PLAN.md — AI Verification Harness

**Plan number:** 12-5-06
**Wave:** 3 — Integration + Regression
**Phase:** 12.5 — Combat AI Intelligence Foundation
**Goal:** Smoke tests that catch regressions in the AI stack without needing a live game.
**Status:** Ready
...
### Step 1: Create `scripts/verify_combat_ai.py`
...
def run_logic_tests() -> list[TestResult]:
    ...
    try:
        ...
        from core.combat_situation import CombatSituationModel
        ...
        from core.movement_policy import MovementPolicy
        ...
        from core.death_classifier import DeathClassifier
...
def run_telemetry_tests() -> list[TestResult]:
    ...
    try:
        sys.path.insert(0, str(SRC_DIR))
        from services.match_telemetry import MatchTelemetry, CombatTick, MatchSummary
        import tempfile
        import json
...
def run_regression_tests() -> list[TestResult]:
    ...
    try:
        sys.path.insert(0, str(SRC_DIR))
        from services.discord_event_service import (
            emit_event,
            should_dispatch,
    ...
    try:
        with open(SRC_DIR / "core" / "bot_engine.py", "r") as f:
            content = f.read()
        checks = [
...
Expected: All tests pass. If any fail, fix before calling 12.5 complete.
...
- All compile tests pass (7 modules)
- All import tests pass (all symbols found)
- All config tests pass (combat_ai keys present, deep_merge preserves existing)
...
- All telemetry tests pass (JSONL writes, summary writes, get_last_ticks)
- All regression tests pass (Phase 12.4 Discord hooks still in bot_engine.py)
- Phase 12.5 is complete only when all checks pass
